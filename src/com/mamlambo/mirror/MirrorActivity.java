...
public class MirrorActivity extends Activity implements PictureCallback {
  ...
  /**
   * 
   * @return O identificador da primeira câmera frontal encontrada.
   */
  private int findFirstFrontFacingCamera() {
    ...
    for (int camId = 0; camId < numCams; camId++) {
      ...
      if (info.facing == CameraInfo.CAMERA_FACING_FRONT) {
        Log.d(DEBUG_TAG, "Found front facing camera (id="+camId+")");
        foundId = camId;
        break;
      }
   ...
   * 
   * @param layout
   * @param cameraId Identificador da primeira câmera frontal
   */
  ...
  public void onPictureTaken(byte[] data, Camera camera) {
    ...
    try {
      ...
      if(bitmap!=null) {
        Log.d(DEBUG_TAG,"size: "+bitmap.getWidth()+" x "+bitmap.getHeight());
      }
  ...
  public class MirrorView extends SurfaceView implements SurfaceHolder.Callback {
    ...
    public void setCameraDisplayOrientationAndSize() {
      ...
      */
      ...
      Log.d(TAG,"toString(): "+previewSize.toString());
      Log.d(TAG,"w: "+previewSize.width);
      Log.d(TAG,"h: "+previewSize.height);
